...
功能
----
1. 递归扫描 RAG/templates/ 的所有子目录，自动索引所有 .xml 模板文件。
2. 每个子目录为一个"分类"（category），通过 meta.json 提供元数据描述。
3. 提供基于关键词的模糊检索（无需向量数据库，纯文本匹配）。
4. 提供模板内容读取接口，供 MCP 工具层调用。
5. 提供 save_generated_xml()，将 AI 生成的 XML 持久保存到 RAG/generated/ 目录。
...
GENERATED_DIR = _THIS_DIR / "generated"
...
def save_generated_xml(block_name: str, xml_content: str) -> Path:
    """
    将 AI 生成的 XML 内容保存到 RAG/generated/ 目录。
